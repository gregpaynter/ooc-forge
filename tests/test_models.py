...
def test_reference_upscale_model_is_pinned_to_official_realesrgan_release():
    assert REFERENCE_UPSCALE_MODEL["id"] == "realesrgan-x4plus"
    assert REFERENCE_UPSCALE_MODEL["filename"] == "RealESRGAN_x4plus.pth"
    assert REFERENCE_UPSCALE_MODEL["scale"] == 4
    assert REFERENCE_UPSCALE_MODEL["sha256"] == "4fa0d38905f75ac06eb49a7951b426670021be3018265fd191d2125df9d682f1"
    assert REFERENCE_UPSCALE_MODEL["source_url"] == (
...
def test_model_install_status_defaults_to_idle(tmp_path):
    ...
    assert model_install_status(config)["state"] == "IDLE"
    assert upscale_model_install_status(config)["state"] == "IDLE"
...
def test_model_install_status_reads_persistent_status(tmp_path):
    ...
    upscale_path = tmp_path / "maintenance" / "upscale-model-install-status.json"
    upscale_path.write_text(json.dumps({"state": "COMPLETED", "message": "ready"}), encoding="utf-8")
    assert upscale_model_install_status(config)["state"] == "COMPLETED"
...
def test_request_reference_upscale_model_install_is_non_blocking(monkeypatch, tmp_path):
    config = make_config(tmp_path)
    calls = []
    ...
    assert command == [
        ...
        "--no-block",
        "start",
        "ooc-forge-upscale-model-install.service",
    ]
    assert kwargs["check"] is True
    assert kwargs["timeout"] == 10
